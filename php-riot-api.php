...
PHP Riot API Wrapper
James Lee (c) 2016

Anyone may revise and use this free of charge.
...
class riotapi {
	...
	const API_URL_CHAMP_M = 'https://{region}.api.pvp.net/championmastery/location/{platformId}/player/';
	...
	//Returns specific champion according to its given id.
	public function getThisChampion($id){
		$call = 'champion/'.$id;
	...
	//Returns specific champion skin set.
	public function getChampionSkins($id){
		$call = 'champion/'.$id;

		//add API URL to the call
		$call = self::API_URL_STATIC_1_2 . $call;
		$call .= '?champData=skins';
	...
	//Returns current game information for player on platform (region?)
	...
	//Returns a user's matchHistory given their summoner id and champion id
	public function getChampionHistory($id, $champId) {
		$others = '?championIds='. $champId .'&seasons=SEASON2015,PRESEASON2016,SEASON2016';
	...
	//Returns a summoner's stats given summoner id (of Season 6)
	public function getStats($id,$option='summary'){
		$call = 'stats/by-summoner/' . $id . '/' . $option . '?season=SEASON2016';
	...
	//returns a summoner's profile icon id
	public function  getSummonerProfileIconId($id) {
		$summoner = $this->getSummoner($id);
		if (self::DECODE_ENABLED) {
			return $summoner[$id]['profileIconId'];
		}
		else {
	...
	// Return's a summoner's Champion Mastery from their summoner id and region/platform
	public function getMastery($id, $platform) {

		$call = str_replace("{platformId}", $platform, self::API_URL_CHAMP_M);
		$call .= $id . "/topchampions";
